                                    ...
                                    <form id="formSubmit">
                                        ...
                                        <div class="form-group">
                                            <label class="col-sm-3 control-label no-padding-right">Chi tiết
                                                quyền</label>
                                            <div class="col-sm-9">
                                                <c:forEach var="item" items="${roleDetailPermission}">
                                                    <input id="${item}" type="checkbox" name="listStringRoleDetailName"
                                                        value="${item}" <c:forEach var="item1"
                                                        items="${groupRoleRoleDetails}">
                                        ...
                                        <div class="form-group">
                                            <div class="col-sm-12">
                                                <c:if test="${not empty groupRoles.id}">
                                                    <input type="button" class="btn btn-white btn-warning btn-bold"
                                                        value="Cập nhật nhóm quyền" id="btnAddOrUpdateGroupRole" />
                                                </c:if>
                                                <c:if test="${empty groupRoles.id}">
                                                    <input type="button" class="btn btn-white btn-warning btn-bold"
                                                        value="Thêm nhóm quyền" id="btnAddOrUpdateGroupRole" />
                                                </c:if>
                                            </div>
                                        </div>
                                        <input type="hidden" value="${groupRoles.id}" id="id" name="id" />
                                    </form>
                                </div>
                            </div>
                        </div>
        ...
        <script>
            ...
            function addGroupRole(data) {
                ...
                data.forEach(element => {
                    ...
                    if (element.name == "code")
                        code = element.value;
                    if (element.name == "listStringRoleDetailName")
                        grRoleDetail.push(element.value);
                    if (element.name == "id")
                        id = element.value;
                });
                sendDta = {
                    name: name,
                    code: code,
                    listStringRoleDetailName: grRoleDetail,
                    id: id
                };
                $.ajax({
                    url: '/api-admin-grouprole',
                    type: 'POST',
                    contentType: 'application/json',
                    data: JSON.stringify(sendDta),
                    dataType: 'json',
                    success: function (result) {
                        window.location.href = "/admin-addOrUpdateGroupRole?id=" + result.id + "&message=insert_success";
                    },
                    error: function (error) {
                        window.location.href = "/admin-grouprole?message=error_system";
                    }
                });
            }
            function updateGroupRole(data) {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    success: function (result) {
                        window.location.href = "/admin-addOrUpdateGroupRole?id=" + result.id + "&message=update_success";
                    },
                    error: function (error) {
                        window.location.href = "/admin-grouprole?message=error_system";
                    }
